...
A bullish reversal pattern that consists of three candles:
1. A large bearish candle in a downtrend
2. A doji candle that gaps down
3. A bullish candle that closes well into the body of the first candle
"""
...
class MorningDojiStarPattern(BasePattern):
    ...
    The Morning Doji Star pattern consists of:
    - A large bearish candle in a downtrend
    - A doji candle (small body) with a gap down from the first candle
    - A bullish candle that closes well into the body of the first candle
    - Indicates a potentially strong bullish reversal
    """
    ...
    def detect(self, candles):
        """
        Detect Morning Doji Star pattern in candlestick data.
        ...
        # Check Morning Doji Star criteria
        
        # 1. First candle must be bearish with a substantial body
        ...
        # 3. Check for gap down between first candle and doji
        first_body_low = first_candle['close']  # First candle is bearish
        doji_body_high = max(doji_candle['open'], doji_candle['close'])
        
        # Calculate gap as percentage
        gap_percent = (first_body_low - doji_body_high) / first_body_low * 100
        
        if gap_percent < self.min_gap_percent:
            # Allow some flexibility if there's a clear separation
            if doji_body_high >= first_body_low:
                return None
        
        # 4. Third candle must be bullish with significant penetration into first candle
        if not self.is_bullish_candle(third_candle):
            return None
            
        # Calculate penetration ratio
        first_body_high = first_candle['open']  # First candle is bearish
        first_body_size = first_body_high - first_body_low
        
        penetration = third_candle['close'] - first_body_low
        penetration_ratio = penetration / first_body_size if first_body_size > 0 else 0
        
        if penetration_ratio < self.min_reversal_ratio:
            return None
        
        # 5. Determine if we're in a downtrend (for reversal context)
    ...
    def _create_pattern_result(self, candles, first_candle, doji_candle, third_candle, 
        ...
        result = {
            ...
            'notes': f"Morning Doji Star at ${third_candle['close']:.2f}, {strength_desc} {context}"
        }

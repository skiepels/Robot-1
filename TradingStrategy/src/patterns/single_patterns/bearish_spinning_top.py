...
with wicks on both sides. In a downtrend, it can indicate potential continuation.
"""

from ..base_pattern import BasePattern


class BearishSpinningTopPattern(BasePattern):
    """
    Detects the Bearish Spinning Top candlestick pattern.
    
    A Bearish Spinning Top is characterized by:
    - Small bearish (red) body
    - Located in the middle of the candle's trading range
    - Upper and lower wicks of similar size (relatively balanced)
    - Appears in a downtrend or at resistance levels
    """
    ...
    def detect(self, candles):
        """
        Detect Bearish Spinning Top pattern in candlestick data.
        ...
        if not self.is_bearish_candle(current_candle):
            return None
        ...
        # Check Bearish Spinning Top criteria
        ...
        body_low = current_candle['close']
        body_high = current_candle['open']
    ...
    def _create_pattern_result(self, candles, spinning_top, confidence, trend):
        ...
        spinning_top: pandas.Series
            The Bearish Spinning Top candle
        confidence: float
        ...
        if trend == "downtrend":
            direction = "bearish"  # Continuation in downtrend
        
        # For trading parameters, we'll use a breakdown approach
        # Entry below the spinning top low
        entry_price = spinning_top['low'] * 0.999
        
        # Stop above the spinning top high
        stop_price = spinning_top['high'] * 1.001
        ...
        result = {
            ...
            'notes': f"Bearish Spinning Top at ${spinning_top['close']:.2f} with {confidence:.1f}% confidence"
        }
